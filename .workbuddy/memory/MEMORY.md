...
## 改動前必查
- **報表頁**：KPI 帶**固定 `grid-cols-5`**（10 格 5-5）；**唔可以**用 `md:grid-cols-3 xl:grid-cols-5`（iPad 768–1279 落 3 格 → 3-3-3-1），10 格要**同一個 grid**。`:key` remount ≠ 自動刷新（`dataReady` 歸 false → 全頁閃 skeleton），要用 `refreshToken`。
- **`normalizePosLocalSettings` 係白名單重建** → 加欄唔加白名單 = 靜靜剷走（中過：`qrUrl`/`paperSize`/`shiftPresets`）。
- **掃碼雙模式（docs/115）**：`/menu?tableId=`（堂食每枱一碼）/ `/quick?store=`（快餐全店一碼）link 完全區隔；`/menu` 冇 tableId **唔可以**當快餐落單。模式 `scan_mode` **由登入驅動**：設定頁唯讀，唯一寫入點 `login-screen.tsx`（`saveAuthSession()` 後、導航前 `await`）；`kiosk`/`salon` **唔寫**。
- **快餐掃碼用店內 `pickup` 序號**（唔可以用台名「自取」做單號＝全店同號），**唔 resume**；離線用 `quickScanOfflineOrderNo()`，**唔可以**用 `nextLocalDailyOrderNo()`。
- **`saveKioskSettings(storeId, patch, headers?)`**（唔再係 boolean）；`/api/pos/kiosk-settings` POST 係 **read-then-merge** + 42703 降級。**「未經授權：需要 POS 終端憑證。」＝ client 冇帶／冇續期 token（非權限問題）**：`posDeviceAuthHeaders()` 只讀唔續期（TTL 12h）→ 用 `posDeviceAuthHeadersFresh()`。
...
- **持續型提示唔可以照抄 `setToast`**（2.6s 自動清）；要 store-scope localStorage + 只喺 realtime `onOrderUpsert` 由 `isNewSelfOrder` 觸發（**唔可以寫死 `source==="scan"`**）；位置 `top-20`、容器 `pointer-events-none`；標識「有真枱→台名；counter→單號」。
