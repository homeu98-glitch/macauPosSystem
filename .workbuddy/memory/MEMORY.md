...
- 🔴 **出紙修正「改咗代碼但行為唔變」＝ 幾乎一定係 APK 冇重 build**（docs/101/102/103/114）。**不只要重 build，仲要擰 `versionCode`**——唔擰就分唔清部機裝咗邊個 build（實例：docs/114 嘅分格線修正 09-10 寫好、一直只喺源碼，部機跑 09-04 APK → 09-11 實紙仍然「一條線變兩條」；反觀網頁會自動更新，所以出現「預覽正常、實紙唔正常」）。驗證：`git log -1` 日期 vs `app/build/outputs/apk/debug/*.apk` mtime vs 工作區 `git status`（`M` = 未 commit）。Android 建置要先俾 `JAVA_HOME`（`/c/Program Files/Android/Android Studio/jbr`），否則 `ERROR: JAVA_HOME is not set`；SDK 路徑喺各 repo `local.properties`。
