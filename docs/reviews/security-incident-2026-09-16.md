...
> ✅ **2026-09-17 覆核：已 commit 且已部署。**
> - commit：**`b910a76`**（09-16 21:35 +08）
> - Vercel Production deployment：`ref=b910a76`（created 09-16 13:36:38 UTC = 21:36 +08）
> - server 實際使用：`sync/route.ts:13`（import）、`:499`（LWW 預取）、`:682-684`（主解析）
>
> 本節原本寫「本地工作區，未 commit、未部署」——該句寫於 20:50 之前，**現已不成立**。
