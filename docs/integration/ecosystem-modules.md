...
- 充值 / 派送 / ADR 詳細契約：見 [Macau-Ledger](https://github.com/EricChang1015/Macau-Ledger) repo 的 `docs/` 目錄
- 外賣派單：[macauMemebershipDeliveryDriver](https://github.com/homeu98-glitch/macauMemebershipDeliveryDriver)
- 商戶 Android App：[macau-ledger-merchant](https://github.com/EricChang1015/macau-ledger-merchant)
- **第三方 POS ↔ Ledger 整合**：[ledger-client-api.md](./ledger-client-api.md) → [macauPosSystem](https://github.com/homeu98-glitch/macauPosSystem)
...
| **macauPosSystem** | **獨立 Supabase**（店內 POS）；商戶 Ledger 登入後 **client 直連 Ledger Supabase**（訂單可寫 RPC + Realtime；報表／菜單唯讀）；**無** Ledger webhook／Vercel（見 [ledger-client-api.md](./ledger-client-api.md)） |
...
| **HiveMQ MQTT** | Ledger 下單 publish `notifications`（提示音）、**接單後** publish `jobs`（打印）→ 商戶 App subscribe（ADR-023，見 Macau-Ledger repo） |
| **Web Push（VAPID）** | Ledger 商戶改狀態／派送 callback 後推播至顧客瀏覽器（ADR-027，見 Macau-Ledger repo） |
...
App **不經** Ledger Vercel；以下為 Phase 1 主要 RPC（完整清單見 [Macau-Ledger ADR-024](https://github.com/EricChang1015/Macau-Ledger)）：
...
夥伴 POS **不經** Ledger Vercel；訂單以 **Realtime**（`orders`、`merchant_id` filter）同步為主，重連後 `list_merchant_orders` 增量補洞；**禁止**定時 polling。完整契約見 [ledger-client-api.md](./ledger-client-api.md)。
...
| 文件 | 內容 |
|------|------|
| [ledger-client-api.md](./ledger-client-api.md) | 第三方 POS ↔ Ledger 整合契約 |
| [Macau-Ledger docs](https://github.com/EricChang1015/Macau-Ledger) | ADR-023 MQTT 打印、ADR-024 Android、ADR-025 polling、ADR-029 派送 |
| [../README.md](../README.md) | 本 repo 文檔索引 |
