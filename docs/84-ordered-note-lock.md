...
7. **長備註排版**（§7）：落一張備註係 `少冰，小冰，少飯，少甜，走冰，走蔥，少辣，走甜，走蔥，小冰` 嘅單 →
   備註要喺動作列**下面**獨立一行、向下換行到第 2～3 行；「退 1 份」掣**唔好**被推走位、
   card 右邊界**唔好**被撐破、畫面**唔好**出現橫向捲軸。
8. **全單備註長文**：同上，文字向下換行，「編輯／已鎖定」掣要留喺**頂部**（`items-start`），
   唔好因為文字增高而變垂直居中。
...
`item.note` 過長時（`truncate` 生效）文字會一直向右延伸，`truncate` 只係喺容器有 overflow 約束時先出省略號，
否則文字照樣撐出 card 邊界／喺窄容器產生橫向捲軸。

### 方案 A（已採用）

備註**由「動作列右半」拎出嚟做獨立一行**，整寬顯示、向下自動換行：

```
修改前（向右延伸）              修改後（向下換行）
┌────────────────────┐         ┌────────────────────┐
│ 牛筋麵        [+]  │         │ 牛筋麵        [+]  │
│ 寬版拉麵 · MOP 65  │         │ 寬版拉麵 · MOP 65  │
│                    │         │                    │
│ 編輯備註  備註：少冰│         │ 編輯備註  已退 1 份│
│          小冰，少飯│         │ 備註：少冰，小冰， │
│          少甜…    │         │       少飯，少甜   │
└────────────────────┘         └────────────────────┘
   ↑ 撐出右邊界                   ↑ 整寬、自動斷行
...
- `whitespace-pre-wrap`：保留換行符、自動換行
- `break-words`（`overflow-wrap: break-word`）：**必要**，純 `break-normal` 對長串 CJK 無效
  （CJK 無空白位可斷，會照樣向右撐）

| # | 位置 | 原 | 改 |
| --- | --- | --- | --- |
| 1 | 購物車單品備註 | 喺 `min-w-0 text-right` 內、`truncate` | 拎出動作列 → 獨立 row `mt-1.5 whitespace-pre-wrap break-words` |
| 2 | 全單備註 display | `truncate` ×2 | 外層改 `items-start`、內層 `min-w-0 flex-1`，兩行都改 `whitespace-pre-wrap break-words` |
| 3 | 打印預覽 item 備註 | 無 truncate（窄容器風險） | 加 `whitespace-pre-wrap break-words` |
| 4 | 訂單詳情抽屜 item 備註 | 同上 | 同上 |

### 配套（保証唔走位）

- 動作列淨留掣 + 「已退 N 份」（短、加 `shrink-0`）→ 永遠單行，唔會被長備註擠走
- 全單備註外層 `items-center` → `items-start`：備註換行增高時「編輯／已鎖定」掣留頂部，唔會被拉到垂直居中
- 全單備註文字區 `min-w-0 flex-1`：食晒剩餘寬度並以 card 邊界為限，唔會推擠右邊掣
