...
## 🔴 交班歷史工具列／分頁改版（2026-09-15）

商家三項要求：① 篩選條件排**同一行** ② 移除 Excel 導出、只留 CSV ③ 每頁最多 **10 天** ＋「查看更多」。

- 🔴 **篩選排唔到一行嘅真兇係「副標題太長」**：舊版標題＋副標題＋篩選同喺一個
  `flex items-center justify-between` 容器；副標題係長句 → 左邊 div 食走大部分闊度 →
  右邊 `flex-wrap` 嘅最後一粒掣被迫換行、壓落表頭（用戶截圖「導出 Excel 疊住表頭」）。
  ⇒ 修法係**把標題搬上去自己一行**，篩選另開一條 `.bar`（`flex` + `nowrap` +
  容器太窄時 `overflow-x-auto`，唔會拆行）。
- **口徑**：「一頁 10 天」＝ **10 個澳門日曆日**（一日可以 2–3 個班次 → 行數可以多過 10）；
  「查看更多」＝**累加**（不是換頁）；CSV 導出＝**篩選後全部**（唔受 10 天限制）。
- 🔴 **框架尺寸必須固定**（商家明確要求）：表格容器 `maxHeight: 620px` ＋ `overflow-auto`
  ⇒ 撳「查看更多」內容由 1089px 變 2663px，框仍然 620px（內部滾動）。
- 🔴 **`table-layout:fixed` 之下唔可以靠 `min-w-[150px]` 撐格**（舊寫法用嚟撐備註欄）——
  固定佈局會**忽略**子元素 min-width，反而令內容溢出。備註欄嘅闊度要寫喺 `<colgroup>`。
- 🔴 **欄寬一定要用「真瀏覽器實測」定，唔可以估**：12px 字下
  「MOP 3,945」實測要 **66–70px** 內容寬度 ⇒ 營業額類欄位要 94px；
  欄闊 58px 時會斷成「MOP」/「3,945」兩行（`scrollWidth > clientWidth` **捉唔到**，
  因為佢係「換行」唔係「溢出」——呢個係上次誤判嘅原因）。
  量法：`Range.selectNodeContents(textNode).getBoundingClientRect().width`。
  最終基準 = **1130px**（`min-w-[1130px]`），各欄百分比以 1130 為分母。
- ⚠️ 表頭用 **11px**（`text-[11px]`）：12px 時「應收金額合計」等表頭會斷成
  「應收金額合 / 計」。
- ⚠️ **iPad 橫向（1180 viewport → 內容約 1040px）**下 12 欄一度需要橫向滾動 90px。
  🔴 **2026-09-15 已解**：**貨幣由每格搬上表頭**（表頭黏一行細字 `MOP`，每格只顯示數字）
  ⇒ 每欄省 24–28px，7 個金額欄共省 ~170px ⇒ 基準 1130 → **1018px**，
  iPad 1180 實測**橫向溢出 = 0**（12 欄全部完整、零格被裁；5 位數金額照樣單行）。
  ⚠️ 數字格式化要用 `lib/format.ts` 嘅 `formatMoneyValue()`（同 `formatMoney` 同一套捨入／分位），
  **唔可以**自己寫 `toLocaleString`，否則兩邊口徑漂移。舊 iPad（1024 → 內容 884px）仍會溢 134px。
- **日分隔列**：每日一行「DD/MM/YYYY（週X）· N 個班次」，令「10 天」睇得見。
  🔴 日期一律用 `macauDateKey()`（澳門邊界），**唔可以** `closedAt.slice(0,10)`
  （UTC）—— 澳門 00:00–08:00 收工嘅班次會被算落前一日，同日分隔列／篩選／格內顯示互相矛盾。
- 驗證：真 Chromium 撳「查看更多」10 天 → 20 天 → 確認框高不變；逐格量
  `scrollWidth`／文字 `Range` 闊度；iPad 1180 再掃一次。
...
### 🎥 影片證據（2026-09-14 10:45 商家實拍，7.03s / 720×1280 / 30fps）
用 `imageio-ffmpeg` 抽幀（本機冇 ffmpeg，`pip install imageio-ffmpeg` 自帶 binary）逐幀分析：
- ✅ **撳落去確實有聚焦**：textarea 邊框變橙（`focus:border-orange-400`）、出現文字游標、
  而且**整個欄位有 iOS AutoFill 嘅淡黃底色** —— 呢三樣都只會喺真聚焦時出現。
- ✅ **系統層認得呢個係可編輯文字欄位**：焦點位上彈出系統文字工具列「**貼上｜自動填寫**」
  （撳完 2–3 秒仍在，唔係一閃）。
...
| `pos-app.tsx` 備註彈窗 | textarea 加 `autoFocus`（app 內其餘 5 個輸入框都有，唯獨呢個漏咗）、`text-base`(16px，避開 iOS focus 自動放大 vs `maximumScale:1` 衝突)、`autoCorrect/autoCapitalize/spellCheck` 關（中文組字唔會被自動更正食走）；**`onPointerUp` 明確補 `focus()`**（防 iOS 首次 tap 被當成 scroll／雙擊縮放而唔 focus，已聚焦時係 no-op）＋ `touch-action:manipulation` / `-webkit-user-select:text`；彈窗 z-index `z-50` → **`z-[70]`**（同「免單備註」一致，脫離同層打架） |
...
4. **用 Safari 普通分頁開（唔用「加到主畫面」嘅圖示）**：若普通分頁彈得出鍵盤、PWA 唔得
   ⇒ 係 iOS standalone PWA + `fixed` 彈窗問題，對應做法係改彈窗定位或唔用 standalone。
5. **同一彈窗內其他自由文字框**：免單備註／折扣備註（同款 `fixed` 彈窗嘅 `<input autoFocus>`）
   打唔打得入？打得入 ⇒ 系統鍵盤冇被裝置封死，問題收窄到「備註欄位本身 ＋ 撳落去嘅聚焦」。
   ⚠️ 全部唔得 ＋ 搜尋商品（非彈窗普通 input）都唔得 ⇒ 100% 裝置層。
6. **設定 → 一般 → 鍵盤 → 鍵盤**：清單係咪只剩「表情符號」（＝所有軟鍵盤被移除）。

🔴 **「改完仲係唔彈」第一件事：確認商家跑緊嘅版本**
（收銀機／desktop 載 **Vercel 部署** → 改完**冇 push 就冇生效**；iPad 分頁**唔會自動換 JS**
 → 要**完全關閉分頁重開**，普通 reload 未必夠）。判斷法：睇今次新加嘅行為有冇出現（例如
 備註彈窗嘅「自由輸入」字級變 16px、或彈窗可撳遮罩關閉）。

### 如果裝置層冇得救（未做，待拍板）
「自由文字」係全 app 唯一冇自繪輸入基建嘅類型 → 要根治只有三條路（成本由低到高）：
① **擴大 `設置 → 備註 → 常用備註` chip 庫**（`notePresets`，已支援「撳 chip 疊加」）令日常 95% 情境
   **唔使打字**；② **備註彈窗加「最近用過」chip 行** —— 由本機 `orders` 嘅 `orderNote` / `items[].note`
   推導最近 N 個唔重複備註，一撳即重用（純前端、唔使 DB）；
③ 做備註專用「App 內建文字輸入板」（QWERTY／常用字＋貼上）—— 工程量最大，要先出確認稿。
⚠️ 過渡期可用「貼上」：喺 iPad 備忘錄打好 → 長按備註欄 → 貼上（影片已見「貼上」工具列彈得出＝焦點正常）。
...
### 四個入口（2026-09-15 定案，改任何一個之前先睇齊）

| 線上接單 | 線下接單 |
|---|---|
| ① 桌台總覽標題列（`pos-app.tsx`，`size="xs"`） | ① 桌台總覽標題列（`size="xs"`） |
| ② 訂單頁 › 線上訂單卡（`size="md"`） | ② **訂單頁 › 店內線下訂單卡（`size="md"`）** ← 2026-09-15 新增 |
| ③ 快餐訂單列 › 線上訂單（`size="xs"`） | ③ 快餐訂單列 › 線下訂單（`size="xs"`） |
| ④ 設置 › 線上接單**分頁**（`size="md"`） | ④ 側欄商店名卡（`app-sidebar.tsx`，撳商店名切換） |

🔴 **設置頁 header 嗰粒已經拿走**（2026-09-15 J 指示）—— `MerchantOrderHeaderToggle` 元件已刪。
設置頁**分頁**嗰粒**保留**（設定場景就地開關 ＋ Ledger 未接通時嘅「重新整理」補救）。
實機驗證：`headerPillCount === 0`、分頁仍然有 `線上接單 160×52` ＋ `自動接單 120×44`。

🔴 **訂單頁係「上下兩區、每區全寬」**（`orders-hub.tsx` `grid-rows-2`），唔係左右分欄 ——
所以兩張卡嘅 header 各有約 **980px** 可用：上區右簇 384px、下區（加咗線下接單之後）312px，
一行放得落。唔好誤以為係半寬而縮細 pill。
