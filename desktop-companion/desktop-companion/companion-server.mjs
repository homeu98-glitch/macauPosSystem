...
// ---- USB 打印機枚舉（Meituan / Sunmi 式「揀型號唔使查 VID/PID」）----
// 經 node-usb 枚舉已連接 USB 設備，按 VID/PID 對應品牌 / 型號 / 字集 / 紙寬，
// 前端只顯示友好名稱、自動帶入 VID/PID，商家全程唔使手填。
//
// ⚠️ PID 多變，優先按 VID→品牌 + 通用「ESC/POS 熱敏」；真實型號由 USB 字串描述元補齊（未做，避免 detach kernel driver）。
//   呢個表 best-effort、可無痛擴充：加一行就得。
//   數值用 10 進制（node-usb 返嘅係 10 進制 VID/PID）。
const USB_PRINTER_DB = [
  { vendorId: 0x04b8, brand: "Epson", model: "Epson ESC/POS 熱敏", defaultCharset: "utf-8", defaultPaper: "80mm" },
  { vendorId: 0x0519, brand: "Star", model: "Star Micronics ESC/POS 熱敏", defaultCharset: "utf-8", defaultPaper: "80mm" },
  { vendorId: 0x04cb, brand: "Citizen", model: "Citizen ESC/POS 熱敏", defaultCharset: "utf-8", defaultPaper: "80mm" },
  { vendorId: 0x1504, brand: "Bixolon", model: "Bixolon ESC/POS 熱敏", defaultCharset: "utf-8", defaultPaper: "80mm" },
  { vendorId: 0x0d90, brand: "Seiko Instruments", model: "SII ESC/POS 熱敏", defaultCharset: "utf-8", defaultPaper: "80mm" },
  { vendorId: 0x0483, brand: "Xprinter", model: "芯烨 Xprinter 熱敏", defaultCharset: "gb18030", defaultPaper: "80mm" },
  { vendorId: 0x0416, brand: "Gprinter", model: "佳博 Gprinter 熱敏", defaultCharset: "gb18030", defaultPaper: "80mm" },
  { vendorId: 0x1fc9, brand: "Zjiang", model: "中崎 Zjiang 熱敏", defaultCharset: "gb18030", defaultPaper: "80mm" },
  { vendorId: 0x1a03, brand: "Gprinter", model: "佳博 Gprinter 熱敏", defaultCharset: "gb18030", defaultPaper: "80mm" },
  { vendorId: 0x2bdf, brand: "Rongta", model: "榮達 Rongta 熱敏", defaultCharset: "gb18030", defaultPaper: "80mm" },
  { vendorId: 0x04b4, brand: "Cypress", model: "USB 打印晶片（白牌）熱敏", defaultCharset: "gb18030", defaultPaper: "80mm" },
];
const KNOWN_USB_PRINTER_VIDS = new Set(USB_PRINTER_DB.map((m) => m.vendorId));

function resolveUsbMeta(vendorId, productId) {
  const hit = USB_PRINTER_DB.find((m) => m.vendorId === vendorId);
  if (hit) {
    return { brand: hit.brand, model: hit.model, defaultCharset: hit.defaultCharset, defaultPaper: hit.defaultPaper, recognized: true };
  }
  const vidHex = "0x" + (vendorId || 0).toString(16).toUpperCase().padStart(4, "0");
  const pidHex = productId != null ? "0x" + productId.toString(16).toUpperCase().padStart(4, "0") : "";
  return {
    brand: "未知",
    model: `未識別 USB（${vidHex}:${pidHex}）`,
...
function enumerateUsbPrinters() {
  return new Promise((resolve) => {
    if (!usbLib) {
      resolve({ printers: [], note: "companion 未安裝 usb 套件（USB 打印/偵測停用）" });
      return;
    }
    try {
      ...
      const all = list
        .map((dev) => {
          ...
          return {
            ...
            brand: meta.brand,
            model: meta.model,
            charset: meta.defaultCharset,
            paperSize: meta.defaultPaper,
            connectionType: "usb",
            recognized: meta.recognized,
          };
        })
        .filter(Boolean);
      // 有識別到已知打印機 VID → 只返呢啲；否則返全部（標 recognized:false）等商家按插拔確認
      const known = all.filter((d) => d.recognized);
      const printers = known.length ? known : all;
      const note = known.length ? undefined : all.length ? "未識別 USB 設備，請按插拔確認是否打印機" : undefined;
      resolve({ printers, note });
    } catch (e) {
      resolve({ printers: [], note: "USB 枚舉失敗：" + (e?.message || e) });
    }
  });
}
...
function createHandler() {
  return async (req, res) => {
    ...
    // USB 打印機枚舉（node-usb）：返 [{vendorId, productId, brand, model, charset, paperSize, connectionType:"usb"}]
    // 前端「偵測已連接打印機」按鈕 call 呢度；VID/PID 由 Companion 對應型號表自動帶，商家唔使填。
    if (req.method === "GET" && req.url === "/api/usb") {
      const result = await enumerateUsbPrinters();
      res.writeHead(200, { "Content-Type": "application/json" });
      return res.end(JSON.stringify({ ok: true, printers: result.printers, note: result.note }));
    }

    // 合併 LAN(mDNS) + USB 候選（前端「新增打印機」一鍵掃描用）。
    if (req.method === "GET" && req.url === "/api/printers") {
      const [lan, usb] = await Promise.all([
        bonjourLib ? discoverPrinters() : Promise.resolve([]),
      ...
      return res.end(
        JSON.stringify({
          ok: true,
          lan: lan.map((p) => ({ ...p, connectionType: "lan" })),
          usb: usb.printers,
          note: usb.note,
        }),
      );
