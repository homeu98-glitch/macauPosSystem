 ...
 *
 * ## 為何兩條 link 一定要分開（用戶明確要求）
 *
 * | | 堂食 | 快餐 |
 * |---|---|---|
 * | 路由 | `/menu?tableId=<枱>&store=<店>` | `/quick?store=<店>` |
 * | 碼嘅數量 | **每張枱一個** | **全店一個** |
 * | 落單綁定 | 枱號（同一枱再加單 = 改同一張單） | 冇枱（`tableId="counter"`，**每單獨立**） |
 * | 落單號碼 | 唔產生（訂單標識 = 台名） | 店內 `pickup` 序號（同 kiosk 共用，e.g. `自取01`） |
 * | 成功頁 | 「本枱訂單」+ 加單（冇「完成」） | 「已收到」+ 取餐號 + 再點一單 |
 *
 * 兩邊**唔會互相 fallback**：
 *   - `/menu` 冇 `tableId` → **唔會**靜靜當快餐落單，而係顯示「請掃枱上 QR」；
 *   - `/quick` 唔理有冇 `tableId`（快餐本就冇枱）。
 * 舊版 `/menu?store=` （冇枱）係會當快餐落單嘅（docs/115 G1 缺口）—— 兩條 link 撈埋一齊，
 * 日後改堂食流程一定誤傷快餐，所以家陣直接喺呢度閘死。
 *
 * ⚠️ 兩者仍然共用 `useScanOrder()`（同一套中性基礎設施：餐牌 bootstrap、售罄、realtime、
 * 購物車、金額、落單重試／隊列）。真正唔同嘅係「落單號碼 / resume / 成功頁」，
 * 全部收喺 hook 同呢個元件嘅 `quick` 分支，唔會漏。
 */
export type ScanLinkKind = "dine_in" | "quick";

export function ScanOrderPage({ link }: { link: ScanLinkKind }) {
  const {
    hydrated,
    menuLoading,
    menuUnavailable,
  ...
  /** 快餐 link（`/quick`）：全店一碼、冇枱、每單獨立。 */
  const quick = link === "quick";
  ...
  // ── 連結唔完整（docs/115：兩條 link 完全區隔，唔可以互相 fallback）──
  // 堂食 link 冇 tableId：唔可以當快餐落單（否則客人掃錯 / 過期 link 會落一張
  // 「冇枱」嘅單，收銀枱面搵唔到）。快餐 link 冇 store：冇店就冇餐牌。
  if (!storeId || (!quick && !tableId)) {
    return (
      <main className="mx-auto flex min-h-[100dvh] max-w-md flex-col items-center justify-center bg-stone-50 p-6 text-center">
        ...
        <h1 className="mb-2 text-xl font-bold text-stone-900">
          {quick ? "連結不完整" : "請掃描枱上 QR 點餐"}
        </h1>
        <p className="max-w-sm text-sm text-stone-500">
          {quick
            ? "手機點餐需由店內「快餐區 / 櫃檯」嘅點餐 QR 開啟，請重新掃碼。"
  ...
  // ── 快餐成功頁（docs/115 G3 修復）──
  //
  // 快餐冇「本枱訂單」（`activeTableOrder` 對 quick 永遠係 null），所以舊版落單成功後
  ...
  // ── 本枱訂單（**堂食 only**：剛落單成功 + 重複掃碼載入既有訂單，兩者共用）──
  //
  ...
  if (!quick && activeTableOrder && !ordering) {
  ...
  return (
    <main className="mx-auto flex h-[100dvh] w-full max-w-md flex-col overflow-hidden bg-stone-50">
      ...
      {/* 本枱現有訂單（加單模式下置頂提醒）—— 客人端一律唔顯示單號（快餐冇本枱單） */}
      {activeTableOrder && (
