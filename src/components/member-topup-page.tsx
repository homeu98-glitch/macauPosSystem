        ...
        <header className="border-b border-slate-200 bg-white px-4 py-3">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <div className="mt-1 text-sm text-slate-500">
                線上轉帳截圖審核（topUpAutomation 店主後台）。開啟「自動核准」後由伺服器每分鐘執行，無須一直停留此頁。
              </div>
            </div>
            {state.status === "ready" ? (
          ...
          {state.status === "ready" ? (
            <div className="mt-3 rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-900">
              ...
              <div className="mt-1 text-emerald-800">POS 登入帳號：{state.staffAccount}</div>
            </div>
          ) : null}
        </header>
            ...
            <>
              {iframeBlocked ? (
                <div className="mb-3 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
                  嵌入顯示失敗時，請使用「在新分頁開啟」，或確認 topUp 已部署 iframe 支援（SameSite=None Cookie）。
                </div>
              ) : null}
              <iframe
                key={state.embedUrl}
