 ...
 *
 * ## 點解係 10 分鐘 → **2026-09-21 改成 60 分鐘**
 *
 * 回執過期 → 重新入工作集 → 守護 pull 一次雲端核實 → 一致就即刻寫返新回執。
 *
 * ⚠️ **但實測（2026-09-21 Supabase log）成本唔係「一次 pull」，係 ⌈N／100⌉ 次**：
 *  `MAX_ORDERS_PER_ROUND = 100`，而每輪都會重新拉一次**全店訂單**
 *  （舊版仲要係 `limit=5000` + 冇日期下限 ≈ 每次 7 MB）。10 分鐘 TTL 之下，
 *  每個週期要拉 ⌈N/100⌉ 次 —— 日營 30 張單嘅店已經要每 10 分鐘拉 3 次，
 *  換算約 **1.2 GB／日**（與 2026-09-21 實測 1.29 / 1.47 GB／日 吻合）。
 *
 * ⇒ 商業取捨：由「雲端被回水最多 10 分鐘內被發現」放寬到「最多 60 分鐘」，
 *    換取拉取次數直接變 **1/6**。對單店餐飲完全可接受（人手亦可隨時開
 *    「同步健康」Modal 即刻核實，嗰條路唔受 TTL 限制）。
 *    要還原舊行為：把下面常數改返 `10 * 60 * 1000` 就得。
 *
